...
export function CardDetailSheet({ card }: { card: CardType }) {
	...
	const [watchedTitle, watchedDescription, watchedDueDate] = useWatch({
	...
	const hasChanges = useMemo(() => {
		...
		const normalizeDueDate = (v?: string | null) => (v ? v : null);

		const currentTitle = normalizeText(watchedTitle);
		const currentDescription = normalizeText(watchedDescription);
		const currentDueDate = normalizeDueDate(watchedDueDate);

		const initialTitle = normalizeText(card.title ?? "");
		const initialDescription = normalizeText(card.description ?? "");
		const initialDueDate = normalizeDueDate(card.dueDate ?? null);
					...
					>
						...
						<SheetFooter className="flex-col gap-2 p-0 sm:flex-row">
							...
							<Button
								type="submit"
								size="lg"
								disabled={!hasChanges || update.isPending || remove.isPending}
							>
								Update
							</Button>
						</SheetFooter>
